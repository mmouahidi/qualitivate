...
export async function seed(knex: Knex): Promise<void> {
  // Insert the 5Q food safety template only if it doesn't already exist
  const existing = await knex('survey_templates').where('name', '5Q food safety and quality culture survey').count('id as count');
  if (Number(existing[0].count) > 0) {
    console.log('5Q food safety template already exists, skipping.');
    return;
  }
  ...
  console.log('Inserted 5Q food safety template');
}
